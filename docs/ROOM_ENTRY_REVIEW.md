...
| シナリオ | Safe Update 挙動 |
|----------|------------------|
| 部屋にいる間（waiting 含む全フェーズ） | バナー表示のみ（ボタンなし）、自動適用は抑止 |
| 部屋を退出（RoomPage アンマウント） | in-game hold 解放、自動更新可能 |

#### ゲーム中の手動適用ボタン

**ゲーム中（suppressed 状態）は手動適用ボタンを表示しない。**

8人プレイ中に誰かが誤って「今すぐ適用」を押してゲームが中断されるリスクを防ぐため、
バナーには情報のみ表示し、ボタンは表示しない。
メインメニューに戻ると自動的に更新が適用される。
