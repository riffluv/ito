...
## 7. ゲーム進行中の抑止ガイド
- プレイ中（ラウンド進行中）は `holdInGameAutoApply()` を送って自動適用を抑止し、待機・メインメニューに戻ったら `releaseInGameAutoApply()` で解除する。BroadcastChannel 経由で全タブ同期される。  
- バナー文言は抑止中「自動適用を停止中」と表示され、手動適用を押せば解除して適用を進められる。  
- デプロイ頻度が高い場合も、待機に戻った瞬間に自動適用（または手動適用）できるため、ゲーム中の強制リロードを防ぎつつ最新化を維持できる。
