...
const leaveRoomAsHost = async (
  ...
  const response = await page.request.post(`${origin}/api/rooms/${roomId}/leave`, {
    data: {
      uid,
      token,
      displayName,
  ...
  expect(response.ok(), `leave route failed (${response.status()})`).toBe(true);
  const json = await response.json();
  expect(json.ok).toBe(true);
};

test("ホスト退出後にプレイヤーがホストへ移譲される", async ({ page, browser }) => {
  ...
  try {
    ...
    await leaveRoomAsHost(page, origin, roomId, hostName);
    await page.close().catch(() => undefined);

    const startButton = p2.page.getByRole("button", { name: "ゲーム開始" });
    await expect(startButton).toBeVisible({ timeout: 60_000 });
    await expect(startButton).toBeEnabled({ timeout: 60_000 });

    await expect(p2.page.getByText(/参加人数：1人/)).toBeVisible({ timeout: 60_000 });
  } finally {
