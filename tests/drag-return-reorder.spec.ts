...
const getPlayerIdFromWaiting = async (page: Page, playerName: string) => {
  ...
  await expect(waitingCard).toBeVisible({ timeout: 45_000 });
  const id = await waitingCard.getAttribute("data-player-id");
  if (!id) {
    throw new Error(`Failed to read data-player-id for ${playerName}`);
  }
  return id;
};
...
const getSlotIndexForCard = async (
  page: Page,
  cardId: string,
  slots: ReturnType<Page["locator"]>
) => {
...
const expectCardInSlot = async (page: Page, cardId: string, slotIndex: number) => {
  ...
  const token = boardRoot.locator(`[data-card-id="${cardId}"]`).first();
...
const waitForRemovedFromBoard = async (pages: Page[], cardId: string, timeoutMs = 60_000) => {
  await expect
    .poll(
      async () =>
        Promise.all(
          pages.map((p) =>
            getSlotIndexForCard(p, cardId, p.locator("[data-board-root] [data-slot]"))
          )
        ),
      { timeout: timeoutMs }
    )
    .toEqual(pages.map(() => null));
};

test("戻す→再配置でも完走できる", async ({ page, browser }) => {
  ...
  try {
    ...
    const hostCardId = await getPlayerIdFromWaiting(page, hostName);
    const p2CardId = await getPlayerIdFromWaiting(page, p2Name);
    const p3CardId = await getPlayerIdFromWaiting(page, p3Name);
    ...
    for (const p of [page, p2.page, p3.page]) {
      ...
      await expectCardInSlot(p, p2CardId, midIdx);
      await expectCardInSlot(p, p3CardId, rightIdx);
    }

    const p2Return = p2.page.getByRole("button", { name: "戻す" }).first();
    await expect(p2Return).toBeVisible({ timeout: 30_000 });
    await expect(p2Return).toBeEnabled({ timeout: 30_000 });
    await p2Return.click();
    await waitForRemovedFromBoard([page, p2.page, p3.page], p2CardId, 60_000);

    const hostReturn = page.getByRole("button", { name: "戻す" }).first();
    await expect(hostReturn).toBeVisible({ timeout: 30_000 });
    await expect(hostReturn).toBeEnabled({ timeout: 30_000 });
    ...
    for (const p of [page, p2.page, p3.page]) {
      ...
      await expectCardInSlot(p, p2CardId, leftIdx);
      await expectCardInSlot(p, p3CardId, rightIdx);
    }
