/**
 * ホストが不意切断した場合でも自動クレームでホスト移譲されるかをE2Eで検証
...
const waitForHostMissing = async (page: Page, timeoutMs = 60_000) => {
  await page.waitForFunction(
    () => (window.__ITO_METRICS__?.participants?.activeCount ?? 0) <= 1,
    null,
    { timeout: timeoutMs }
  );
};
...
test("ホスト不意切断でも自動クレームでホスト移譲される", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(guest.page.getByTitle(`ホスト: ${hostName}`)).toBeVisible({
      timeout: 30_000,
    });

    await page.context().setOffline(true);

    await waitForHostMissing(guest.page, 60_000);
    await waitForHostLikelyUnavailable(guest.page, 90_000);

    await expect(guest.page.getByTitle(`ホスト: ${guestName}`)).toBeVisible({
      timeout: 90_000,
    });
  } finally {
