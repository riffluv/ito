/**
 * タブを閉じてすぐ復帰しても、プレイヤーが観戦扱いにならないことを確認
...
test("タブ閉じ→復帰でも観戦にならない", async ({ page, browser }) => {
  ...
  try {
    ...
    const reopened = await openRoomInContext(p2.context, origin, roomId);
    pages.push(reopened);
    await waitForPhase(reopened, "clue", 60_000);
    await expect(reopened.getByText("▼ 観戦中 ▼")).toBeHidden({ timeout: 60_000 });
    await expect(reopened.getByLabel("連想ワード")).toBeEnabled({ timeout: 60_000 });
  } finally {
