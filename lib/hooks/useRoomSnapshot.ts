...
function applyPlayerPatch(
  ...
  const clue1 =
    typeof patch.clue1 === "string" ? patch.clue1 : undefined;
...
function mergePlayersWithOptimisticPatches(
...
const ROOM_SNAPSHOT_POST_STALE_MS = (() => {
  const parsed = Number(
...
const ROOM_SNAPSHOT_CACHE_ONLY_STALE_MS = (() => {
  const parsed = Number(
...
const ROOM_SNAPSHOT_RECOVERY_HARD_COOLDOWN_MS = (() => {
  const parsed = Number(
