...
export function PlayBoard({
  ...
  const onPlay = async () => {
    ...
    try {
      ...
      toast({ title: "出せませんでした", description: e?.message, status: "error" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Panel title="順番に出す">
      <Stack>
        {!failed ? (
          <Text color="gray.300">低いと思った人から順に「出す」を押してください。順番を間違えると失敗になります。</Text>
        ) : (
          <Text color="red.300" fontWeight="bold">失敗扱いです。最後まで出して数字を確認しましょう。</Text>
        )}
        ...
        <Stack>
          ...
          {played.length === 0 ? (
            ...
            <Stack>
              {played.map((p, idx) => (
                <HStack key={p.id} justify="space-between" p={2} borderWidth="1px" rounded="md" bg={failed && failedAt === (idx + 1) ? "red.900" : undefined}>
            ...
            <Stack>
              {waiting.map((p) => (
                <HStack key={p.id} justify="space-between" p={2} borderWidth="1px" rounded="md">
                  <Text>{p.name}</Text>
                  {p.id === meId ? (
                    <Button onClick={onPlay} colorScheme="blue" isLoading={submitting} isDisabled={!canPlay}>
                      出す
                    </Button>
                  ) : (
