...
export const BaseCard = forwardRef<HTMLDivElement, BaseCardProps>(
  ({ variant = "empty", size = "md", children, css: cssProp, ...props }, ref) => {
    ...
    const usesGlobalCardSize = variant === "empty" && size === "md";
    const globalWidth = { base: "var(--card-w-base, 100px)", md: "var(--card-w-md, 120px)" };
    const globalHeight = { base: "var(--card-h-base, 140px)", md: "var(--card-h-md, 168px)" };

    return (
      <Box
        ref={ref}
        // ✅ レスポンシブサイズ適用 (!important除去)
        width={usesGlobalCardSize ? globalWidth : { base: sizeConfig.width.base, md: sizeConfig.width.md }}
        height={usesGlobalCardSize ? globalHeight : { base: sizeConfig.height.base, md: sizeConfig.height.md }}
        minWidth={usesGlobalCardSize ? globalWidth : { base: sizeConfig.width.base, md: sizeConfig.width.md }}
        minHeight={usesGlobalCardSize ? globalHeight : { base: sizeConfig.height.base, md: sizeConfig.height.md }}
        {...variantStyles}
        {...props}
        css={{
          ...(usesGlobalCardSize
