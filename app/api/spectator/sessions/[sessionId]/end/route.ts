...
type SessionEndOverrides = {
  auth?: ReturnType<typeof getAdminAuth>;
  db?: ReturnType<typeof getAdminDb>;
  now?: () => number;
};

let overrides: SessionEndOverrides | null = null;
...
declare global {
  ...
  var __setSpectatorSessionEndOverrides:
    | ((value: SessionEndOverrides | null) => void)
    | undefined;
}

globalThis.__setSpectatorSessionEndOverrides = (value) => {
...
export async function POST(
  ...
  try {
    ...
    logError("spectator", "session-end-auth-failed", { sessionId, roomId, error });
  ...
  try {
    ...
    logDebug("spectator", "session-end-success", {
      sessionId,
      roomId,
      viewerUid,
      reason: reason ?? null,
    ...
    logError("spectator", "session-end-error", { sessionId, roomId, error });
