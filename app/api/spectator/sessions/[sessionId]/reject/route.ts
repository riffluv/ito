...
type SessionRejectOverrides = {
  auth?: ReturnType<typeof getAdminAuth>;
  db?: ReturnType<typeof getAdminDb>;
  now?: () => number;
};

let overrides: SessionRejectOverrides | null = null;
...
declare global {
  ...
  var __setSpectatorSessionRejectOverrides:
    | ((value: SessionRejectOverrides | null) => void)
    | undefined;
}

globalThis.__setSpectatorSessionRejectOverrides = (value) => {
...
export async function POST(
  ...
  try {
    ...
    logError("spectator", "session-reject-auth-failed", { sessionId, roomId, error });
  ...
  try {
    ...
    logDebug("spectator", "session-rejoin-rejected", {
      sessionId,
      roomId,
    ...
    logError("spectator", "session-reject-error", { sessionId, roomId, error });
