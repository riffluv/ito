...
type SessionWatchOverrides = {
  auth?: ReturnType<typeof getAdminAuth>;
  db?: ReturnType<typeof getAdminDb>;
  now?: () => number;
};

let overrides: SessionWatchOverrides | null = null;
...
declare global {
  ...
  var __setSpectatorSessionWatchOverrides:
    | ((value: SessionWatchOverrides | null) => void)
    | undefined;
}

globalThis.__setSpectatorSessionWatchOverrides = (value) => {
...
export async function POST(
  ...
  try {
    ...
    logError("spectator", "session-watch-auth-failed", { sessionId, roomId, error });
  ...
  try {
    ...
    logDebug("spectator", "session-watch-success", { sessionId, roomId, viewerUid });

    return NextResponse.json({ ok: true }, { status: 200 });
  } catch (error) {
    logError("spectator", "session-watch-error", { sessionId, roomId, error });
