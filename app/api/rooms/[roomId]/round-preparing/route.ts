...
import { setRoundPreparingCommand } from "@/lib/server/roomCommands";
...
export async function POST(req: NextRequest, { params }: { params: { roomId: string } }) {
  ...
  try {
    await setRoundPreparingCommand({ roomId, token: parsed.data.token, active: parsed.data.active });
    return NextResponse.json({ ok: true });
  } catch (error) {
    traceError("room.roundPreparing.api", error, { roomId });
