...
import { pruneProposalCommand } from "@/lib/server/roomCommands";
...
const schema = z.object({
  ...
  eligibleIds: z.array(z.string()).default([]),
...
export async function POST(req: NextRequest, { params }: { params: { roomId: string } }) {
  ...
  try {
    await pruneProposalCommand({
      roomId,
      token: parsed.data.token,
      eligibleIds: parsed.data.eligibleIds ?? [],
    });
    return NextResponse.json({ ok: true });
  } catch (error) {
    traceError("room.pruneProposal.api", error, { roomId });
