...
import { mutateProposal } from "@/lib/server/roomCommands";
...
const schema = z.object({
  ...
  targetIndex: z.number().optional().nullable(),
...
export async function POST(req: NextRequest, { params }: { params: { roomId: string } }) {
  ...
  try {
    const status = await mutateProposal({
      roomId,
      token: parsed.data.token,
      playerId: parsed.data.playerId,
      action: parsed.data.action,
      targetIndex: parsed.data.targetIndex ?? undefined,
    });
    return NextResponse.json({ ok: true, status });
  } catch (error) {
    traceError("room.proposal.api", error, { roomId });
