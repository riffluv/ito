...
import { continueAfterFailCommand } from "@/lib/server/roomCommands";
...
export async function POST(req: NextRequest, { params }: { params: { roomId: string } }) {
  ...
  try {
    await continueAfterFailCommand({ roomId, token: parsed.data.token });
    return NextResponse.json({ ok: true });
  } catch (error) {
    traceError("room.continue.api", error, { roomId });
