...
import { updateRoomOptionsCommand } from "@/lib/server/roomCommands";
...
const schema = z.object({
  ...
  resolveMode: z.string().optional().nullable(),
  defaultTopicType: z.string().optional().nullable(),
...
export async function POST(req: NextRequest, { params }: { params: { roomId: string } }) {
  ...
  try {
    await updateRoomOptionsCommand({
      roomId,
      token: parsed.data.token,
      resolveMode: parsed.data.resolveMode ?? undefined,
      defaultTopicType: parsed.data.defaultTopicType ?? undefined,
    });
    return NextResponse.json({ ok: true });
  } catch (error) {
    traceError("room.options.api", error, { roomId });
