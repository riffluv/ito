...
import { finalizeRevealCommand } from "@/lib/server/roomCommands";
...
export async function POST(req: NextRequest, { params }: { params: { roomId: string } }) {
  ...
  try {
    await finalizeRevealCommand({ roomId, token: parsed.data.token });
    return NextResponse.json({ ok: true });
  } catch (error) {
    traceError("room.finalize.api", error, { roomId });
